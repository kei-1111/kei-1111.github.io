...
/** GitHub の1日分のコントリビューション。level は 0..4（Less → More）。 */
@Serializable
...
@Serializable
private data class ContributionsResponse(
    val total: Map<String, Int>,
    val contributions: List<ContributionDay>,
)

private const val CONTRIBUTIONS_API = "https://github-contributions-api.jogruber.de/v4/"
...
internal fun parseContributions(body: String): ContributionCalendar? = try {
    val response = json.decodeFromString<ContributionsResponse>(body)
    ContributionCalendar(
        totalLastYear = response.total.values.sum(),
        days = response.contributions,
    )
...
/**
 * 実データを非同期に取得して返す。取得中は null、取得失敗時は静的スナップショットにフォールバックする。
 */
@Composable
internal fun rememberContributionCalendar(user: String): ContributionCalendar? =
    produceState<ContributionCalendar?>(initialValue = null, user) {
        val live = fetchText("$CONTRIBUTIONS_API$user?y=last")?.let(::parseContributions)
        value = live ?: FallbackContributions.calendar
    }.value
