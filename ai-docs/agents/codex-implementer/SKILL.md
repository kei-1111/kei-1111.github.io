...
Return: Sol's final summary (condensed), the verify result and fix rounds used, the session id (the caller needs it for delta rounds), changed files with a one-line note each, every comment the change adds with its justification (or "no comments added"), the rules the brief declared it covers (echoed back), any files changed outside the brief's stated scope, and any failure output verbatim. Review and detekt validation belong to the caller.
