...
2. **Write the brief** — `codex exec` opens a fresh session with no memory of this conversation, so the brief must be self-contained:
   ...
   - What NOT to touch
3. **Delegate** — write the brief to a file (scratchpad or temp directory) and pass its path to the `codex-implementer` subagent (contract: `ai-docs/agents/cross-agent/codex-implementer/SKILL.md`). Delegate one brief at a time — parallel runs share the working tree and corrupt each other's change attribution.
...
5. **Validate** — run the narrowest relevant validation (`./gradlew :feature:<name>:compileKotlinWasmJs`, `./gradlew detekt` — rerun once if autoCorrect reformats).
6. **Iterate or take over** — send review/validation failures back to Sol as a delta brief. A delta brief opens a fresh session too, so it must stay self-contained: restate the goal, target files, and constraints, then state what is wrong, in which file, and what correct looks like. After two failed rounds on the same problem, stop delegating: fix it directly and note the takeover in the report.
