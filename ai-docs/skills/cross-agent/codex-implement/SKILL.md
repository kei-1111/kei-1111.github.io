...
2. **Write the brief** — `codex exec` opens a fresh session with no memory of this conversation, so the brief must be self-contained:
   ...
   - Constraints, transferred mechanically rather than from recall (Codex reads `AGENTS.md`, not `.claude/rules/`): run `scripts/list_matching_rules.sh <planned target files>` to enumerate the applicable rules (always-loaded + `paths:`-matched), and inline each rule's applicable sections into the brief; end the brief with the list of rules it covers so the implementer can echo it back
   - What NOT to touch
