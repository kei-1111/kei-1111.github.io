...
リモート Repository は `app:core:data` から自作 API を呼び出します。取得できない場合（オフライン・タイムアウト・サーバーダウン・Android Preview ターゲット上での実行）は Flow が例外を投げ、ViewModel の `.asResult()` が `Result.Error` に変換して、UI がエラー表示＋再試行を描画します。サーバー側はさらに GitHub 公式 GraphQL API からライブデータを取得し、静的コンテンツと合成します。正確なエンドポイントと対象データはソースコードを正本とします。
...
4. `toState()` は `contributionsResult` が `Result.Success` のときだけ `State.contributions` に値を入れ、`Result.Error` は `contributionsLoadFailed` フラグとして公開する。取得中はスケルトン／ビルド中インジケータを、失敗時はエラー行＋再試行リンク（`RetryBackendData`）を Preview 側が描画する
