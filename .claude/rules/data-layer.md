...
- A new persisted user setting follows the same shape as theme/language — Repository write + `app:core:local` data source over the shared settings store; never a raw `localStorage` access elsewhere.
