...
Canonical detail: `.claude/rules/ci-cd.md` (path-scoped to `.github/**` — the workflow files, CD pipelines, Playwright cache warming, docs-only gate). In short: every PR to `main` runs the always-on script checks plus detekt / compile / test workflows that skip for docs-only changes; merging a PR deploys immediately.
