...
- CI is 5 independent workflow files, each triggered on every PR to `main`: `check-ai-docs-structure.yml` (`./scripts/check_ai_docs.sh`) always runs; `detekt.yml`, `compile-wasm.yml`, `compile-android.yml`, `server-test.yml` each run `./gradlew detekt` / `:app:webApp:compileKotlinWasmJs` / `compileAndroidMain` / `:server:test` respectively (JDK 21, temurin; autoCorrect disabled on CI). A `PreToolUse` hook (`.claude/hooks/pre-push-detekt.sh`, wired in `.claude/settings.json`) runs `./gradlew detekt` before any `git push` and blocks the push unless it passes cleanly — an autoCorrect reformat also blocks, since the committed code would still fail CI; commit the formatting fix and push again.
