...
- CI is 5 independent workflow files, each triggered on every PR to `main`: `ai-docs.yml` (`./scripts/check_ai_docs.sh`) always runs; `detekt.yml`, `compile-wasm.yml`, `compile-android.yml`, `server-test.yml` each run `./gradlew detekt` / `:app:webApp:compileKotlinWasmJs` / `compileAndroidMain` / `:server:test` respectively (JDK 21, temurin; autoCorrect disabled on CI). Run `./gradlew detekt` locally before pushing — local autoCorrect may reformat on the first run, so re-run until it passes.
- CD (`.github/workflows/cd-app.yml` + `cd-server.yml`): each is a gate job (`changes`) followed by a `build` job then a `deploy` job (`needs:`). `cd-app.yml` builds `:app:webApp:wasmJsBrowserDistribution` on push to `main` and deploys it to GitHub Pages, skipping server-only changes (`paths-ignore`); `cd-server.yml` builds `:server:buildFatJar` on pushes touching `server/**`/`shared/**`/build config and deploys it to Cloud Run via Workload Identity Federation. Merging a PR deploys immediately — a PR must build and pass detekt before merge.
- Docs-only gate: the 4 gated CI files and both CD files call the reusable `detect-docs-only.yml`, which resolves the PR's linked Issue (`Closes #123`) and skips the heavy job when its title starts with `[Documentation]:`. Any missing/unresolvable step (no Issue reference, API failure) fails open and runs normally — the gate job itself failing also falls open, since the gated jobs check `!= 'false'` rather than `== 'true'`. A skipped-by-`if:` job still satisfies required status checks, so docs-only PRs remain mergeable. `ai-docs.yml` is never gated.
