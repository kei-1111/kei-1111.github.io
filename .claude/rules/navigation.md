...
| `navigation/{Feature}NavigationExtensions.kt` | `fun NavBackStack<NavKey>.navigateXxx() = add(Xxx)` extensions — but a destination leaving a startup sequence replaces the stack (`clear()` + `add(Xxx)`) instead. Omit when nothing navigates to the feature's destinations (e.g. `Splash`) |
