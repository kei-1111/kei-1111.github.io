...
| `:template` golden sources or the destination templates | `scripts/generate_destination_templates.sh --check` + `./gradlew :template:compileKotlinWasmJs :template:compileAndroidMain :test:architecture:test` |
| Architecture checks (`test/architecture`) | `./gradlew :test:architecture:test` |
