...
- Metro does not aggregate `@ContributesBinding` contributions from transitive `implementation` dependencies — the contributing module must be a direct dependency of the graph-owning module or be exposed via `api` (Metro FAQ). This is why `app:webApp` depends directly on `app:core:data` even though only `app:core:domain` calls its Repositories
