...
- Apply `.asResult()` where the UseCase `Flow` is collected, and keep the whole `Result` in `ViewModelState` (e.g. `profileResult: Result<Profile> = Result.Loading`), not just the unwrapped data. Reference: `app/feature/profile/.../destination/profile/ProfileViewModel.kt`.
...
- `toState()` unwraps `Success` into the data fields (`Loading` surfaces as `null` = "no data yet") and folds each part's `Result` into a display phase (`.../destination/profile/model/LoadPhase.kt`) — one for the selected page, one each for the Contributions, TODO, and changelog sections. The Profile UI renders those phases as per-part states — editor code skeleton, Preview building indicator, and an error row whose retry dispatches `ProfileIntent.RetryBackendData`, which re-collects only the streams whose `Result` is `Error`. The one silent stream is `terminalCommandsResult`: it has no phase or error UI — on `Error` the terminal simply lacks the server-defined commands (builtins keep working) while `RetryBackendData` still re-collects it.
