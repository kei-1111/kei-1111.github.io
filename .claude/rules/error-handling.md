...
- `toState()` unwraps `Success` into the data fields (`Loading` surfaces as `null` = "no data yet") and derives failure flags from `Error` (`profileLoadFailed` / `contributionsLoadFailed` / `issuesLoadFailed` / `worksLoadFailed` / `readmeLoadFailed`). The Profile UI renders these as per-part states — editor code skeleton, Preview building indicator, and an error row whose retry dispatches `ProfileIntent.RetryBackendData`, which re-collects only the streams whose `Result` is `Error`. The one silent stream is `terminalCommandsResult`: it has no failure flag or error UI — on `Error` the terminal simply lacks the server-defined commands (builtins keep working) while `RetryBackendData` still re-collects it.
