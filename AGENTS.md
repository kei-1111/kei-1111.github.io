...
- `app/core/utils/` — `openUrl` expect/actual (wasmJs: `window.open`, android: no-op), plus `rememberIsPageVisible` / `prefersReducedMotion` and the `VerticalResizeCursor` / `HorizontalResizeCursor` pointer icons (wasmJs: `PointerIcon.fromKeyword`, android: default cursor)
