...
| Unit-tested logic (`app:core:domain` UseCases, `app:core:mvi`, feature ViewModels) | `./gradlew :<module>:testAndroidHostTest` (client unit tests; CI runs them) |
...
- Test suites: `:server:test` (JUnit 5 + Ktor `testApplication` + `MockEngine`; CI runs it) per `.claude/rules/server-testing.md`; the client unit tests (`commonTest` with hand-written fakes, run as Android host tests via `testAndroidHostTest` on `app:core:domain` / `app:core:mvi` / `app:feature:profile`; CI runs them) per `.claude/rules/app-testing.md` with ViewModel specifics in `.claude/rules/mvi-testing.md`; `:test:e2e` (Playwright against a served build, gated on `-PbaseUrl`; CI runs it via `ui-test.yml`) per `.claude/rules/ui-testing.md`. New logic on both the client and `:server` follows TDD per `.claude/rules/tdd.md`.
