 ...
 * wasmJs では User-Agent から [visitorDeviceLabel] で「ブラウザ 主バージョン (OS) wasmJs」を組み立てる。
 */
expect fun visitorDeviceLabel(): String

/** User-Agent 文字列からのラベル組み立て本体。expect は UA の取得だけを担い、判定はここで行う。 */
fun visitorDeviceLabel(userAgent: String): String =
    listOfNotNull(detectBrowser(userAgent), detectOs(userAgent)?.let { "($it)" }, "wasmJs").joinToString(" ")
