...
Canonical detail: `.claude/rules/data-layer.md` (Repository shape, fetch & failure propagation), `.claude/rules/error-handling.md` (`Result<T>` boundary), `.claude/rules/usecase.md` (UseCase shape and Metro bindings). Testing: the Unit Testing And TDD section below.
...
- Client unit tests live in `commonTest` and run as Android host tests: `./gradlew :<module>:testAndroidHostTest` (CI runs them; currently `app:core:domain`, `app:core:mvi`, `app:feature:splash`, `app:feature:profile`).
