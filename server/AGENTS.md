...
- Profile source content lives in `ProfileContent.kt`; the wasm client keeps a preview duplicate in `app/feature/profile`'s `ProfilePreviewFixtures.kt` — update both together (canonical: `.claude/rules/naming-conventions.md` — Text Content).
