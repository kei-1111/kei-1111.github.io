...
に限られる。本スクリプトは「実際に使われている文字＋常用漢字全字」を
残す形で各 TTF を **同一ファイル名のまま in-place** サブセット化し、
転送量を大幅に削減する。常用漢字を丸ごと含めるのは、文言の追加・修正の
たびに再生成が必要になる運用を避けるため（常用漢字内の変更なら再生成不要）。
...
## 再生成すべきタイミング
- 画面に表示する文字列に **常用漢字外の文字**（異体字・人名漢字（例: 髙, 﨑）や
  特殊記号）を追加したとき。常用漢字・かな・ASCII・主要記号の範囲内なら
  再生成は不要（keep-set に最初から全字入っている）
- フォント自体を新しいバージョンに差し替えたとき

注意: 文字の抽出は *.kt の文字列リテラル走査のみで行うため、実行時に
ネットワークから取得するテキスト（例: 将来の Issue #23/#24 の API 駆動の
作品ページ）の文字はこのスクリプトでは拾えない。常用漢字内なら問題なく
表示されるが、常用漢字外の字を API 側で使う場合はこのファイルの
keep-set（JOYO_KANJI_FILE への追記または FIXED_RANGES 拡張）に
手動で加えて再生成する必要がある。
...
2. 常用漢字: scripts/joyo_kanji.txt の 2,136 字を全て残す。日常的な文言変更で
   再生成が不要になるようにするための固定リスト（漢字グリフを持たない
   JetBrains Mono に対しては pyftsubset が黙って無視するだけで無害）。
3. 動的範囲: `core/`, `feature/`, `composeApp/` 配下の *.kt を走査し、
   ...
   全て keep-set に加える。
4. OpenType のレイアウト機能（kern 等）は pyftsubset のデフォルトのまま
...
JOYO_KANJI_FILE = SCRIPT_DIR / "joyo_kanji.txt"
...
def load_joyo_kanji() -> set[str]:
    """scripts/joyo_kanji.txt（1行1字、# 行はコメント）から常用漢字 2,136 字を読み込む。"""
    if not JOYO_KANJI_FILE.exists():
        raise SystemExit(f"{JOYO_KANJI_FILE} が見つかりません。常用漢字リストなしでの実行は危険なため中止します。")
    chars: set[str] = set()
    for line in JOYO_KANJI_FILE.read_text(encoding="utf-8").splitlines():
        line = line.strip()
        if not line or line.startswith("#"):
            continue
        chars.add(line)
    if len(chars) < 2000:
        raise SystemExit(f"{JOYO_KANJI_FILE} の文字数が {len(chars)} しかありません（期待: 2,136）。破損の可能性があるため中止します。")
    return chars
...
def build_keep_set() -> set[int]:
    ...
    keep.update(ord(c) for c in load_joyo_kanji())
