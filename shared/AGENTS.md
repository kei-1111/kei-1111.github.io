...
- Wire-compatibility rules are canonical in the `GitHubProfile.kt` KDoc — client (GitHub Pages) and server (Cloud Run) deploy independently, so read it before changing any `@Serializable` model.
